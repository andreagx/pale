...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{const cache=await caches.open(CACHE);try{const response=await fetch(event.request,{cache:'no-store'});if(response.ok){await cache.put(event.request,response.clone());return response;}}catch(_){} return await cache.match(event.request)||Response.error();})());
});
